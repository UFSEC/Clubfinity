...
// 6. Return error codes instead of throw exception
  ...
  exports.createUser = (
    ...
    const newUser = new User({ name: { first: firstname_, last: lastname_, }, dob: dob_, username: username_, email: email_, password: password_ });
  ...
  exports.updateUser = (username_, updatedInfo) => {
    User.findOneAndUpdate({ username: username_ }, updatedInfo, { upsert:true }, function(error, document) {
        if (error)
          throw error;
