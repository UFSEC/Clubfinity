...
exports.update = (req,res)=>{
    ...
    if(Object.keys(username).length != 0) {
      ...
      if (props.length == 6 && props[0] == 'firstName' && props[1] == 'lastName' && props[2] == 'dob' && props[3] == 'email' && props[4] == 'username' && props[5] == 'password'){
        userDAO.updateUser(username.id, { name: { first: userInfo.firstName, last: userInfo.lastName }, dob: userInfo.dob, username: userInfo.username, email: userInfo.email, password: userInfo.password } );
        res.status(204).send();
      } else {
